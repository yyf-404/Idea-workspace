...
public class UserRedisCacheWriter implements RedisCacheWriter {

    private final RedisConnectionFactory connectionFactory;
    private final Duration sleepTime;

    public UserRedisCacheWriter(RedisConnectionFactory connectionFactory) {
        this(connectionFactory, Duration.ZERO);
    }

    public UserRedisCacheWriter(RedisConnectionFactory connectionFactory, Duration sleepTime) {
    ...
    public void put(String name, byte[] key, byte[] value, @Nullable Duration ttl) {
        ...
        LoggerUtil.getLogger(getClass()).info("put(name="+name+" key="+key);
        double timeOut= Math.random()*6000;
        this.execute(name, (connection) -> {
            if (shouldExpireWithin(ttl)) {
                connection.set(key, value, Expiration.from(ttl.toMillis()+(int)timeOut, TimeUnit.MILLISECONDS), SetOption.upsert());
            } else {
